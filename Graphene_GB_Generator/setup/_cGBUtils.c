...
static char periodicClosest_docstring[] =  "Input: p1, p2. For each point in p1, returns the index and distance of the point in p2 that its closest to with periodic boundaries imposed.";
static char selfClosest_docstring[] =  "Input: p1. For each point in p1, returns the index of the point in p1 that its closest to.";

static PyObject * cGBUtils_vorCentroid(PyObject *self, PyObject *args);
static PyObject * cGBUtils_closest(PyObject *self, PyObject *args);
static PyObject * cGBUtils_periodicClosest(PyObject *self, PyObject *args);
static PyObject * cGBUtils_selfClosest(PyObject *self, PyObject *args);

static PyMethodDef module_methods[] = {
	{"vorCentroid", cGBUtils_vorCentroid, METH_VARARGS, vorCentroid_docstring},
	{"closest", cGBUtils_closest, METH_VARARGS, closest_docstring},
	{"periodicClosest", cGBUtils_periodicClosest, METH_VARARGS, periodicClosest_docstring},
	{"selfClosest", cGBUtils_selfClosest, METH_VARARGS, selfClosest_docstring},
	{NULL, NULL, 0, NULL}
};


/* --- Python 3 模块定义结构体 --- */
static struct PyModuleDef _cGBUtilsmodule = {
    PyModuleDef_HEAD_INIT,
    "_cGBUtils",   /* 模块名，必须与 setup.py 一致 */
    module_docstring, 
    -1, 
    module_methods
};
...
/* --- Python 3 入口函数 --- */
PyMODINIT_FUNC PyInit__cGBUtils(void) 
{
    PyObject *m = PyModule_Create(&_cGBUtilsmodule);
    if (m == NULL)
        return NULL;
...
static PyObject *cGBUtils_periodicClosest(PyObject *self, PyObject *args)
{
	double lx=0.f, ly=0.f;
	// all except regions are ndarrays, regions is a list of lists.
	PyObject *p1_x, *p1_y, *p2_x, *p2_y, *ind_out, *dist_out;
	PyObject *p1_x_arr, *p1_y_arr, *p2_x_arr, *p2_y_arr, *ind_out_arr, *dist_out_arr;
	
	// Get list from input arguments
	if (! PyArg_ParseTuple( args, "ddO!O!O!O!O!O!", &lx, &ly, &PyArray_Type, &p1_x, &PyArray_Type, &p1_y, &PyArray_Type, &p2_x, &PyArray_Type, &p2_y, &PyArray_Type, &ind_out, &PyArray_Type, &dist_out)) return NULL;
	...
	for(iP1 =0; iP1<nP1; iP1++)
	{
		...
		{
			double dx = (ct_p1[0][iP1] - ct_p2[0][iP2]);
			double dy = (ct_p1[1][iP1] - ct_p2[1][iP2]);
			...
			if(iP2 == 0)	minDist = dist;	// Initialize

			minInd = dist < minDist ? iP2 : minInd;
			minDist= dist < minDist ? dist : minDist;
		}
...
static PyObject *cGBUtils_closest(PyObject *self, PyObject *args)
{
	...
	// Get list from input arguments
	if (! PyArg_ParseTuple( args, "O!O!O!O!O!O!", &PyArray_Type, &p1_x, &PyArray_Type, &p1_y, &PyArray_Type, &p2_x, &PyArray_Type, &p2_y, &PyArray_Type, &ind_out, &PyArray_Type, &dist_out)) return NULL;
	...
	for(pt=start; pt<end; pt++)
	{
		...
		int isOpen = 0;
		{
			...
			for(j=0; j<vert_list_len; j++)
			{
				...
				int vert_num_int = PyLong_AsLong(vert_num);
